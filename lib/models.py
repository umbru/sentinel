...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, umbrud):
        golist = umbrud.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_umbrud(umbrud, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_umbrud(self, umbrud, rec):
        import decimal
        import umbrulib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from umbrud...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from umbrud, with every run
        ...
        # get/create, then sync payment amounts, etc. from umbrud - Umbrud is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(umbrud)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from umbrud! %s" % e)
            govobj.vote_delete(umbrud)
            return (govobj, None)
    ...
    def vote_delete(self, umbrud):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(umbrud, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, umbrud, signal, outcome):
        import umbrulib
        ...
        output = umbrud.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = umbrulib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(umbrud, signal)

    def sync_network_vote(self, umbrud, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = umbrud.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 umbru addr, non-multisig
            if not umbrulib.is_valid_umbru_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Umbru address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Umbru Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (umbrulib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not umbrulib.is_valid_umbru_address(addr, config.network):
    ...
    def hash(self):
        import umbrulib
        return umbrulib.hashit(self.serialise())

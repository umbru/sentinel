...
def is_valid_umbru_address(address, network='mainnet'):
    ...
    umbru_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Umbru address
        return False

    if (address_version != umbru_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('umbru.conf')

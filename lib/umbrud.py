...
class UmbruDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some umbrud methods
    ...
    @classmethod
    def from_umbru_conf(self, umbru_dot_conf):
        from umbru_config import UmbruConfig
        config_text = UmbruConfig.slurp_config_file(umbru_dot_conf)
        creds = UmbruConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = umbrulib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = umbrulib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = umbrulib.blocks_to_seconds(diff)

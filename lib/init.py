...
def has_umbru_conf():
    ...
    valid_umbru_conf = False

    # ensure umbru_conf exists & readable
    #
    # if not, print a message stating that Umbru Core must be installed and
    # configured, including JSONRPC access in umbru.conf
    try:
        f = io.open(config.umbru_conf)
        valid_umbru_conf = True
    except IOError as e:
        print(e)

    return valid_umbru_conf
...
def main():
    ...
    if not has_umbru_conf():
        print("Umbru Core must be installed and configured, including JSONRPC access in umbru.conf")
        sys.exit(1)

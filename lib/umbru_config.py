...
class UmbruConfig():
    ...
    def slurp_config_file(self, filename):
        # read umbru.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is umbru.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from umbru.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Umbru defaults...
        default_port = 12354 if (network == 'mainnet') else 22354

        # use default port for network if not specified in umbru.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from umbru.conf

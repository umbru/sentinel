...
def mn_list():
    ...
    masternodelist_full = {
        u'701854b26809343704ab31d1c45abc08f9f83c5c2bd503a9d5716ef3c0cda857-1': u'  ENABLED 70201 yjaFS6dudxUTxYPTDB9BYd1Nv4vMJXm3vK 1474157572    82842 1474152618  71111 52.90.74.124:12353',
        u'f68a2e5d64f4a9be7ff8d0fbd9059dcd3ce98ad7a19a9260d1d6709127ffac56-1': u'  ENABLED 70201 yUuAsYCnG5XrjgsGvRwcDqPhgLUnzNfe8L 1474157732  1590425 1474155175  71122 [2604:a880:800:a1::9b:0]:12353',
        u'656695ed867e193490261bea74783f0a39329ff634a10a9fb6f131807eeca744-1': u'  ENABLED 70201 yepN97UoBLoP2hzWnwWGRVTcWtw1niKwcB 1474157704   824622 1474152571  71110 178.62.203.249:12353',
    }
...
def mn_status_good():
    ...
    status = {
        ...
        "service": "[2604:a880:800:a1::9b:0]:12353",
...
def test_valid_umbru_address():
    from umbrulib import is_valid_umbru_address

    main = valid_umbru_address()
    test = valid_umbru_address('testnet')

    assert is_valid_umbru_address(main) is True
    assert is_valid_umbru_address(main, 'mainnet') is True
    assert is_valid_umbru_address(main, 'testnet') is False

    assert is_valid_umbru_address(test) is False
    assert is_valid_umbru_address(test, 'mainnet') is False
    assert is_valid_umbru_address(test, 'testnet') is True


def test_invalid_umbru_address():
    from umbrulib import is_valid_umbru_address

    main = invalid_umbru_address()
    test = invalid_umbru_address('testnet')

    assert is_valid_umbru_address(main) is False
    assert is_valid_umbru_address(main, 'mainnet') is False
    assert is_valid_umbru_address(main, 'testnet') is False

    assert is_valid_umbru_address(test) is False
    assert is_valid_umbru_address(test, 'mainnet') is False
    assert is_valid_umbru_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from umbrulib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % umbrulib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(umbrulib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(umbrulib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(umbrulib.blocks_to_seconds(16616)) == 2612035
